...
class DataStore(RoomMemberStore, RoomStore,
    ...
    def _get_active_presence(self, db_conn):
        # TODO: We should probably pull out OFFLINE presence with a non-null
        # status_msg

        # TODO: Do we have any guarentee there won't be multiple entries per
        # user_id?

        sql = (
            "SELECT user_id, state, last_active, last_federation_update,"
            " last_user_sync, status_msg FROM presence_stream WHERE state != ?"
        )
        sql = self.database_engine.convert_param_style(sql)
        ...
        return [
            UserPresenceState(**row)
            for row in self.cursor_to_dict(txn)
        ]
